...
import com.dev.hari.trackme.activities.TrackerActivity;
...
public class TrackerService extends Service implements LocationListener, MyConstants {
...
//        authenticate(email, password);
    ...
    private void loadPreviousStatuses() {
        String transportId = db.getString(PREF_MOBILE_NUMBER);
    ...
    private void logStatusToStorage(Map<String, Object> transportStatus) {
        try {
            File path = new File(Environment.getExternalStoragePublicDirectory(""), "tracker-log.txt");
            if (!path.exists()) {
                path.createNewFile();
            }
    ...
    public void onLocationChanged(Location location) {
        ...
        transportStatus.put(LOCATION_TIME, new Date().getTime());
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            transportStatus.put(BEARING_ACCURACY_DEGREES, location.getBearingAccuracyDegrees());
            transportStatus.put(SPEED_ACCURACY_METER_PER_SECOND, location.getSpeedAccuracyMetersPerSecond());
            transportStatus.put(VERTICAL_ACCURACY_METERS, location.getVerticalAccuracyMeters());
        }
